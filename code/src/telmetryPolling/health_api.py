...
app = Flask(__name__)
CORS(app, resources={r"/*": {"origins": "http://localhost:8080"}})  # Allo0w cross-origin requests from localhost:8000
socketio = SocketIO(app, cors_allowed_origins="http://localhost:8080")  # Allow WebSocket connections from localhost:8000
...
def send_to_agent(metric_name, metric_value):
    ...
    try:
        ...
        # alert = code_executor_agent.generate_reply(messages=[{"role": "user", "content": message}])
        
        CORS(app)  # Allow cross-origin requests
        # socketio = SocketIO(app, cors_allowed_origins="*")  # Allow WebSocket connections
        

        # Send message to the front end through WebSocket
        socketio.emit('anomaly_alert', {'message': message})
